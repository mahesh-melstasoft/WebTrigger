...
export class PushNotificationManager {
    ...
    // Read from NEXT_PUBLIC_VAPID_PUBLIC_KEY injected at build time. Fallback to empty string.
    private vapidPublicKey: string = (typeof process !== 'undefined' && process.env?.NEXT_PUBLIC_VAPID_PUBLIC_KEY) || '';
    ...
    async subscribe(): Promise<PushSubscription | null> {
        ...
        try {
            if (!this.vapidPublicKey) {
                throw new Error('VAPID public key not configured. Set NEXT_PUBLIC_VAPID_PUBLIC_KEY in environment.');
            }
            ...
            const msg = error instanceof Error ? error.message : 'Unknown subscription error';
            throw new Error(`Failed to subscribe to push notifications: ${msg}`);
        }
    }
